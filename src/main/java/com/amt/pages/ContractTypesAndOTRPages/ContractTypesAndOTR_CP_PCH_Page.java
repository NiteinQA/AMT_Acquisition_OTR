...
public class ContractTypesAndOTR_CP_PCH_Page extends TestBase {
	ContractTypesAndOTR_CP_PCH_Page obj_contract_types_CP_PCH_ownbook_calculation_page;
	...
	@FindBy(xpath = "(//p[contains(text(),'Contract Purchase')])[1]")
	private WebElement acq_acq_contractTypes_CP;
	...
	@FindBy(xpath = "//p[normalize-space()='Personal Contract Hire']")
	private WebElement acq_contractTypes_customer_contract_PCH;
	...
	public ContractTypesAndOTR_CP_PCH_Page() {
	...
	public boolean contractTypes_selection_and_OTR_calculation(String sheet_name)
		...
		LO.print(" Customer Contract type option = Personal Contract Hire(BCH) has been selected");
		System.out.println(" Customer Contract type option = Personal Contract Hire(BCH) has been selected");
		ExplicitWait.waitTillLoadingIconDisappears(driver, loading_icon, 200);
...
//		System.out.println("RFL AND FRF from screen is " + rfl_and_frf);
	...
	public boolean contractTypes_and_OTR_selection_CP_PCH_Ownbook_calculation(String sheet_name)
			throws InterruptedException, IOException, UnsupportedFlavorException {
		Click.on(driver, acq_contractTypes, 50);
		ExplicitWait.waitTillLoadingIconDisappears(driver, loading_icon, 200);
		Click.on(driver, acq_acq_contractTypes_CP, 50);
...
//	    act.sendKeys(Keys.TAB, Keys.TAB, Keys.TAB, Keys.TAB, Keys.TAB).sendKeys(Keys.ENTER).build().perform();
		...
		double res=Difference.of_two_Double_Values(subtotal_after_discount_excel, subtotal_after_discount_actual_from_screen);
		if(res<0.2)
          {	flag =true; }
